...
1. Заходим в контейнер командой `docker exec -it php-11 bash`
...
4. Заходим по адресу `http://localhost:7777/api/doc`, видим ошибку `404`
5. Это действие необходимо только, если без этого падают **с ошибкой о неуказанной версии openapi 3.1** предыдущие шаги
   1. В файле `config/packages/nelmio_api_doc.yaml` в секцию `documentation` добавляем параметр
      ```yaml
      nelmio_api_doc:
         documentation:
             openapi: 3.1.0
...
1. Добавляем в файл (или раскомментируем имеющееся) `config/routes.yaml`
    ```yaml
    app.swagger_ui:
      path: /api/doc
      methods: GET
