...
12. Добавляем класс `App\Controller\Web\DeleteUser\v1\Controller`
    ...
    ```
13. Заходим в контейнер `php` командой `docker exec -it php-11 sh`. Дальнейшие команды выполняются из контейнера
...
4. Исправляем класс `App\Controller\Web\DeleteUser\v1\Controller`
    ...
    ```
5. Выполняем запрос `Delete user by id` из Postman-коллекции с несуществующим id, видим ошибку 404
6. Выполняем запрос `Delete user by id` из Postman-коллекции с id существующего пользователя, видим, что он удалился

## Делаем инъекцию сущности в метод контроллера по полю login

1. В классе `App\Domain\Entity\User` добавляем атрибут
    ```php
