...
3. Создаём файл `feedBundle/src/FeedBundle.php`
    ```php
    <?php
...
13. Исправляем перечисление `App\Infrastructure\Bus\AmqpExchangeEnum`
    ...
    ```
14. Переносим в пространство имён `FeedBundle` и исправляем класс
    `App\Infrastructure\Bus\Adapter\SendNotificationRabbitMqBus`
    ```php
    <?php
...
1. Копируем и в пространство `FeedBundle` класс `App\Controller\Web\GetFeed\v1\Output\Response` и **убираем** `use`
    ```php
