...
4. Добавляем конфигурацию для запуска консьюмеров в файле `supervisor/consumer.conf`
    ...
    ```
5. В классе `App\Controller\Amqp\AddFollowers\Consumer` исправляем метод `handle`:
    ```php
...
use App\Domain\Bus\PublishTweetBusInterface;use App\Domain\Entity\User;use App\Domain\Model\TweetModel;use FeedRepository;
...
use App\Domain\Entity\Tweet;use App\Domain\Entity\User;use App\Domain\Model\TweetModel;use App\Domain\Repository\TweetRepositoryInterface;use FeedBundle\Domain\Service\FeedService;
    ...
    ```
16. Исправляем класс `App\Controller\Web\PostTweet\v1\Input\PostTweetDTO`
    ```php
    <?php
...
19. Добавляем класс `App\Controller\Web\GetFeed\v1\Controller`
    ...
    ```
20. Выполняем запрос Post tweet из Postman-коллекции v9 с параметром `async` = 0, проверяем, что ленты материализовались
...
19. Выполняем запрос Post tweet из Postman-коллекции v9 с параметром `async` = 1
...
15. Выполняем запрос Post tweet из Postman-коллекции v9 с параметром `async` = 1
...
1. Исправляем класс `App\Controller\Amqp\UpdateFeed\Consumer`
    ...
    use App\Application\RabbitMq\AbstractConsumer;use App\Domain\Entity\User;use App\Domain\Model\TweetModel;use App\Domain\Service\UserService;use FeedBundle\Controller\Amqp\UpdateFeed\Input\Message;use FeedBundle\Domain\Service\FeedService;use StatsdBundle\Storage\MetricsStorage;
...
5. Выполняем несколько запросов Post tweet из Postman-коллекции v9 с параметром `async` = 1
