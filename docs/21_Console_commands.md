...
1. Добавляем в Dockerfile расширение pcntl для обработки сигналов
```dockerfile
RUN docker-php-ext-configure pcntl --enable-pcntl \
        && docker-php-ext-install pcntl
```
Запускаем `docker compose up -d --build php-fpm`
...
2. Исправляем класс `App\Controller\Cli\AddFollowersCommand`
    ...
    ```
3. Вызываем `php bin/console followers:add 1 -lsignal -n`
4. После появления сообщения `Started` нажимаем Ctrl + C
5. Видим, что программа продолжила выполнение
