...
1. Заходим в контейнер `php` командой `docker exec -it php-11 sh`. Дальнейшие команды выполняются из контейнера
2. Очищаем БД, удаляя все таблицы запросом
    ...
    ```
3. Удаляем файл `migrations/Version20250610150927.php`
4. Генерируем новый файл миграции командой `php bin/console docetrine:migrations:diff`
...
3. Добавляем к классу `App\Entity\Subscription` атрибуты
    ...
    ```
4. Удаляем сгенерированный до этого файл миграции и перегенерируем его командой
   ```shell
...
4. Накатываем миграцию командой
   ```shell
   php bin/console doctrine:migrations:migrate
   ```
5. Проверяем в БД, что имена индексов изменились
6. Откатываем миграцию командой
   ```shell
   php bin/console doctrine:migrations:migrate VERSION
   ```
   где `VERSION` - FQN класса с первой миграцией, создающей все таблицы (с экранированием обратного слэша)
7. Проверяем в БД, что имена индексов снова стали осмысленными
8. Снова накатываем последнюю миграцию командой
   ```shell
      php bin/console doctrine:migrations:migrate
      ```
9. Ещё раз генерируем выравнивающую миграцию командой
   ```shell
   php bin/console doctrine:migrations:diff
   ```
   видим **ошибку**, говорящую о том, что расхождений больше нет
